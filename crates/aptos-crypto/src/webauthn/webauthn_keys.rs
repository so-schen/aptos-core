...
//! This file implements traits for WebAuthn public keys.

#[cfg(any(test, feature = "fuzzing"))]
use crate::{ed25519::Ed25519PublicKey, p256::P256PublicKey, traits::*};
...
/// TODO: Should involve DeserializeKey / SerializeKey?
#[derive(Clone, Serialize)]
pub enum WebAuthnPublicKey {
    /// Ed25519 Public Keys
    Ed25519 {
        /// Ed25519 Public Key
...
// }
//
/// Only evaluate when self and other are same signature scheme, otherwise return false
impl PartialEq<Self> for WebAuthnPublicKey {
    fn eq(&self, other: &Self) -> bool {
        match self {
            WebAuthnPublicKey::Ed25519 {
                public_key: self_public_key,
            } => match other {
                WebAuthnPublicKey::Ed25519 {
                    public_key: other_public_key,
                } => self_public_key.0.to_bytes() == other_public_key.0.to_bytes(),
                WebAuthnPublicKey::P256 { .. } => false,
            },
            WebAuthnPublicKey::P256 {
                public_key: self_public_key,
            } => match other {
                WebAuthnPublicKey::Ed25519 { .. } => false,
                WebAuthnPublicKey::P256 {
                    public_key: other_public_key,
                } => self_public_key.to_bytes() == other_public_key.to_bytes(),
            },
        }
    }
}
    ...
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        match self {
            WebAuthnPublicKey::Ed25519 { public_key, .. } => {
                let encoded_pubkey = public_key.0.to_bytes();
                state.write(&encoded_pubkey);
            },
            WebAuthnPublicKey::P256 { public_key, .. } => {
                let encoded_pubkey = public_key.to_bytes();
                state.write(&encoded_pubkey);
            },
        }
    }
...
impl fmt::Display for WebAuthnPublicKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            WebAuthnPublicKey::Ed25519 { public_key, .. } => {
                write!(f, "{}", hex::encode(public_key.0.as_bytes()))
            },
            WebAuthnPublicKey::P256 { public_key, .. } => {
                /// TODO: where is asBytes for P256?
                write!(f, "{}", hex::encode([0x1]))
            },
        }
    }
}
...
impl fmt::Debug for WebAuthnPublicKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "WebAuthnPublicKey({})", self)
    }
}
//
